...
const Drone = React.memo(({ position, light, setpoint }) => {
	...
	// Geometry for the cone (adjust as needed)
	const geometry = new THREE.ConeGeometry(0.7, 3, 12);
  
	// Compute bounding box for the cone
	geometry.computeBoundingBox();
  
	// Shader material for the cone
	const coneMaterial = new THREE.ShaderMaterial({
	  uniforms: {
		color1: { value: new THREE.Color("yellow") }, // Color at the tip
	...
	return (
	  <group position={position} rotation={[0, zRotation, 0]}>
		...
		{/* Cone for the drone */}
		<Cone args={[0.8, 3, 12]} position={conePosition} rotation={[Math.PI, -(Math.PI / 2), coneRotationAngle]}>
